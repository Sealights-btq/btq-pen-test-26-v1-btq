...
pipeline {
   agent {
    ...
    environment {
        IDENTIFIER = 'btq-btq-pen-test-26-v1.btq.sealights.co'
        tag = "btq-pen-test-26-v1_${params.tag}"
    }
     stages {
                        ...
                        IP = "${IDENTIFIER}"
                                stage("Updating Helm") {
                                    sh script: """
                                        ...
                                        ssh -o StrictHostKeyChecking=no -i ${SSH_KEY_FILE} ${SSH_USER}@internal-btq-pen-test-26-v1.btq.sealights.co 'bash /opt/sealights/install-btq.sh --tag=${env.tag} --buildname=${params.buildname} --labid=${params.labid} --branch=${params.branch} --token=${env.SL_TOKEN} --sl_branch=${params.branch}'
                                    """
                                }
                    }
                }
